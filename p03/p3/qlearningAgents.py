...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        "*** YOUR CODE HERE ***"
        self.Q = util.Counter() # table of action values indexed by state-action pairs
        self.inf = 999999999
    ...
    def computeValueFromQValues(self, state): # max over actions
        """
    ...
    def computeActionFromQValues(self, state):
        ...
        max = (None, -self.inf) # (action, value)
        for action in actions:
            Qvalue = self.getQValue(state, action)
            if Qvalue > max[1]:
                max = (action, Qvalue)
    ...
    def getAction(self, state): 
        ...
        if not actions:
            return None
        heads = util.flipCoin(self.epsilon)
        if heads:
    ...
    def update(self, state, action, nextState, reward): # slide 11
        """
